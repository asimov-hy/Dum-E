...
- `data/models/gesture_recognizer.task` - model artifact; do not touch without explicit approval.
- `data/models/gesture_recognizer.task.sha256` - important checksum-like artifact; docs imply it should be preserved and likely committed.
- `data/test_media/manifest.json` and `data/test_media/README.md` - active Phase 5 regression scaffold; required clips are still honestly missing.
- `docs/mediapipeline_build_plan_v5.md`, `docs/mediapipeline_phase_verification_checklist.md`, `docs/mediapipeline_current_state.md`, `docs/mediapipeline_recording_plan.md` - current/canonical MediaPipeline planning and state docs.
- `docs/known_issues.md` - useful current/historical issue note, but needs review because it appears partly stale.
- `scripts/diagnose_gesture_channel_order.py` - useful diagnostic script, but undocumented and untracked.
...
- `README.md`: top-level README plus `data/motions/README.md`, `data/test_media/README.md`, and ignored `.pytest_cache/README.md`; left unchanged because the real README files have distinct scopes.
...
- `.gitkeep`: `data/manuals/.gitkeep` and `logs/.gitkeep`; left unchanged as intentional tracked placeholders.
- `docs/mediapipeline_build_plan_v5.md` and `docs/mediapipeline_phase_verification_checklist.md`: overlapping phase headings; left unchanged because one is the build plan and one is the verification checklist.
- `docs/mediapipeline_recording_plan.md`, `data/test_media/README.md`, and README recorded-media sections: overlapping content; staged for later consolidation, not safe to merge automatically.
...
- Manifest paths checked: `data/test_media/manifest.json` contains 21 clips, 0 present clips, and 19 missing `required_for_phase5` primary clips.
- CLI/script references checked: README and docs reference `scripts/check_regression_media.py`, `scripts/record_regression_clip.py`, and `scripts/download_gesture_model.py`; `scripts/diagnose_gesture_channel_order.py` is not documented.
...
- `python3 scripts/check_regression_media.py` - passed; reported 21 missing clips and 19 missing required Phase 5 clips.
- `python3 scripts/check_regression_media.py --strict` - expected failure; reported 19 missing required Phase 5 clips.
...
- MEDIUM: `data/models/gesture_recognizer.task.sha256` is untracked even though current docs describe the checksum as stored with the model workflow.
...
- LOW: `scripts/diagnose_gesture_channel_order.py` appears useful but is undocumented and untracked.
...
- Document `scripts/diagnose_gesture_channel_order.py` or move it under a future diagnostics section after user approval.
...
- Decide whether `data/models/gesture_recognizer.task` should remain local-only or be tracked as an artifact.
- Decide whether `data/models/gesture_recognizer.task.sha256` should be committed.
...
- Should `data/models/gesture_recognizer.task.sha256` be committed?
- Should `docs/known_issues.md` be updated in place, archived, or kept as current?
- Should `scripts/diagnose_gesture_channel_order.py` be documented and kept?
...
- The safest next cleanup step is Stage 1: remove ignored generated/cache artifacts locally, then decide the tracking policy for `data/models/gesture_recognizer.task.sha256` and the model artifact.
...
- `data/models/gesture_recognizer.task` is present and untracked.
- `data/models/gesture_recognizer.task.sha256` is present and untracked.
...
- Clarified document ownership:
  ...
  - `docs/mediapipeline_current_state.md` is current MediaPipeline status.
  - `docs/mediapipeline_recording_plan.md` is the required-clip capture plan.
  - `docs/mediapipeline_phase_verification_checklist.md` is the phase checklist.
  - `data/test_media/README.md` is directory-specific media guidance.
...
- `scripts/diagnose_gesture_channel_order.py` was not moved.
- `data/test_media/manifest.json` was not changed.
...
Remaining decisions:
- Decide whether `scripts/diagnose_gesture_channel_order.py` should stay as a
...
Model binary status:
- Path: `data/models/gesture_recognizer.task`
- Exists: yes.
...
Checksum status:
- Path: `data/models/gesture_recognizer.task.sha256`
- Exists: yes.
- Size: 65 bytes.
- Git status: untracked.
- Ignore status: not ignored by project `.gitignore`.
...
- Rationale: `scripts/download_gesture_model.py` can recreate the model from the
...
Checks run:
- `git status --short --branch` - recorded.
- `ls -l data/models/gesture_recognizer.task data/models/gesture_recognizer.task.sha256`
- `git status --short data/models/gesture_recognizer.task data/models/gesture_recognizer.task.sha256`
- `git ls-files data/models/gesture_recognizer.task data/models/gesture_recognizer.task.sha256`
- `git check-ignore -v -- data/models/gesture_recognizer.task data/models/gesture_recognizer.task.sha256`
- `sha256sum data/models/gesture_recognizer.task`
- `cat data/models/gesture_recognizer.task.sha256`
- Reference scans with `rg`.
- `git diff --check` - passed.

Remaining decisions:
- Owner approval to track `data/models/gesture_recognizer.task.sha256`.
- Owner approval for whether to ignore `data/models/gesture_recognizer.task` in
...
- `data/models/gesture_recognizer.task` is ignored by project `.gitignore` and
  remains a local/external artifact.
- `data/models/gesture_recognizer.task.sha256` is not ignored and remains
  visible for future tracking.
- `git status --short --untracked-files=all data/models` reports only
  `data/models/gesture_recognizer.task.sha256` as untracked.

Scripts reviewed:
- `scripts/download_gesture_model.py` remains the setup/verification path for
  the local MediaPipe model. Runtime code must not download the model
  automatically.
- `scripts/check_regression_media.py` remains the hardware-free manifest/media
  status checker. Strict mode is expected to fail while required primary clips
  are missing.
- `scripts/record_regression_clip.py` remains the real-media recording scaffold.
  It should not be treated as normal validation.
- `scripts/diagnose_gesture_channel_order.py` remains diagnostic only for
...
Checks run:
- `git status --short --branch` - recorded.
- `git check-ignore -v data/models/gesture_recognizer.task` - confirmed ignored
  by `.gitignore`.
- `git check-ignore -v data/models/gesture_recognizer.task.sha256 || true` -
  confirmed no ignore rule matched.
- `git status --short --untracked-files=all data/models` - reported only the
...
Remaining decisions:
- Track `data/models/gesture_recognizer.task.sha256` when the owner is ready to
  add project files.
...
- No checklist patch was needed. `docs/mediapipeline_phase_verification_checklist.md`
...
- `docs/mediapipeline_current_state.md`: current/canonical MediaPipeline status;
  do not move.
- `docs/mediapipeline_recording_plan.md`: current MediaPipeline recording plan;
  do not move.
- `docs/mediapipeline_build_plan_v5.md`: planning/reference document still
...
- `docs/mediapipeline_phase_verification_checklist.md`: active phase checklist;
  do not move.
...
- `data/test_media/README.md`: directory-specific current media instructions;
  do not move.
...
- `data/models/gesture_recognizer.task.sha256` should be tracked later if the
  owner approves.
- `data/models/gesture_recognizer.task` should remain ignored/local.
...
- Decide whether to track `data/models/gesture_recognizer.task.sha256`.
...
- Decide later whether `docs/CREATION_LOG.md` or
  `docs/mediapipeline_build_plan_v5.md` should move to an archive/history area
